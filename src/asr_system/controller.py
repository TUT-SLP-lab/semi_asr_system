...
import os
from common.constant import TEXT_OUTPUT, SPLIT_WAV, ASR_MODEL_PATH, ASR_MODEL_CONFIG, OUTLINE_ADDRESS, COLLECTION_NAME
from asr_system.repository.file_io import FileIO


class Controller:
    def __init__(self) -> None:
        self.asr_inference = ASRInference(ASR_MODEL_CONFIG, ASR_MODEL_PATH)
        # self.format_text = FormatText()
        # self.split_audio = SplitAudio()
        self.text_handler = TextHandler(TEXT_OUTPUT, OUTLINE_ADDRESS)
        self.is_running = False

    def speech2text(self, attribute: str, wav_path: str):
        self.is_running = True

        # step1 split audio
        split_wav_list = FileIO.get_all_filepath(SPLIT_WAV, "*.wav")
        print(f"split_wav_list:{split_wav_list}")
        ...
        # TODO split前の音声ファイルによってファイル名を変更
        wav_basename = os.path.basename(wav_path)
        self.text_handler.write_text(hyp_list, f"{TEXT_OUTPUT}/{wav_basename}.txt")
